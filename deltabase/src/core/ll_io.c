...
static int
nb_size(const MetaTable* schema) {
...
read_nb(const MetaTable* schema, int fd) {
...
int
read_dr(const MetaTable* schema,
...
write_mt(const MetaTable* schema, int fd) {
    ...
    w = write(fd, schema->id, sizeof(schema->id));
    if (w != sizeof(schema->id))
        return 1;
    ...
    if (w != sizeof(schema->has_pk))
        return 4;

    w = write(fd, &schema->last_rid, sizeof(schema->last_rid));
    if (w != sizeof(schema->last_rid))
        return 5;
    ...
    if (w != sizeof(schema->columns_count))
        return 7;
