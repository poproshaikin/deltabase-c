...
static int
fmove_pos(size_t pos, size_t offset, int fd) {
...
static int
fmove_neg(unsigned long int pos, long int offset, int fd) {
    ...
    *out_count = 0;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (path_len >= PATH_MAX || path_len < 0) {
            fprintf(stderr,
                    "Error: Path too long or snprintf error for '%s/%s'\n",
                    dir_path,
                    entry->d_name);
            continue;
        }
...
get_directories(const char* dir_path, size_t* out_count) {
    ...
    *out_count = 0;
    ...
    dir_list = (char**)malloc(sizeof(char*) * allocated_size);
    if (dir_list == NULL) {
        perror("Error allocating memory for directory list");
        closedir(dir);
        return NULL;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (stat(full_path, &st) == 0 && S_ISDIR(st.st_mode)) {
            if (count >= allocated_size) {
                allocated_size *= 2;
                char** new_dir_list = (char**)realloc(dir_list, sizeof(char*) * allocated_size);
                if (new_dir_list == NULL) {
                    perror("Error reallocating memory for directory list");
                    free_file_list(dir_list, count);
                    closedir(dir);
                    return NULL;
                }
            ...
            dir_list[count] = (char*)malloc(strlen(full_path) + 1);
            if (dir_list[count] == NULL) {
                perror("Error allocating memory for directory name");
                free_file_list(dir_list, count);
                closedir(dir);
                return NULL;
            }
            strcpy(dir_list[count], full_path);
            count++;
        }
    }
    ...
    *out_count = count;
    ...
    if (count < allocated_size) {
        char** new_dir_list = (char**)realloc(dir_list, sizeof(char*) * count);
        if (new_dir_list != NULL) {
            dir_list = new_dir_list;
        }
    }
...
get_directory_names(const char* dir_path, size_t* out_count) {
    ...
    *out_count = 0;
    ...
    name_list = (char**)malloc(sizeof(char*) * allocated_size);
    if (name_list == NULL) {
        perror("Error allocating memory for directory name list");
        closedir(dir);
        return NULL;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (stat(full_path, &st) == 0 && S_ISDIR(st.st_mode)) {
            if (count >= allocated_size) {
                allocated_size *= 2;
                char** new_name_list = (char**)realloc(name_list, sizeof(char*) * allocated_size);
                if (new_name_list == NULL) {
                    perror("Error reallocating memory for directory name list");
                    free_file_list(name_list, count);
                    closedir(dir);
                    return NULL;
                }
            ...
            name_list[count] = (char*)malloc(strlen(entry->d_name) + 1);
            if (name_list[count] == NULL) {
                perror("Error allocating memory for directory name");
                free_file_list(name_list, count);
                closedir(dir);
                return NULL;
            }
            strcpy(name_list[count], entry->d_name);
            count++;
        }
    }
    ...
    *out_count = count;
    ...
    if (count < allocated_size) {
        char** new_name_list = (char**)realloc(name_list, sizeof(char*) * count);
        if (new_name_list != NULL) {
            name_list = new_name_list;
        }
    }
...
get_executable_dir(char* buffer, size_t buffer_size) {
    ...
    if (strlen(dir) >= buffer_size) {
        fprintf(stderr, "Buffer too small for executable directory\n");
        return -1;
    }
