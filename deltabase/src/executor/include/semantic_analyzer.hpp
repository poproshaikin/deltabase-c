...
namespace exe {
    ...
    class SemanticAnalyzer {
      public:
        SemanticAnalyzer(std::optional<std::string> db_name);
        void
      ...
      private:
        ...
        void
        analyze_where(const std::unique_ptr<sql::AstNode>& where, const MetaTable* table);
        ...
        void
        validate_column_assignment(sql::AstNode* assignment, MetaTable* table);
        MetaColumn*
        ensure_column_exists(const MetaTable* table, std::string colname);
        void
        ensure_db_exists();
        void
