...
namespace exe
{
    QueryPlanner::QueryPlanner(const engine::EngineConfig& cfg, storage::Storage& storage)
    ...
    QueryPlanner::create_plan(const sql::AstNode& node)
    {
        ...
        return std::visit(
            [this](const auto& a) -> QueryPlan
            {
                return this->create_plan(a);
            },
            node.value
        );
    }
    ...
    QueryPlanner::create_plan(const sql::UpdateStatement& stmt)
    {
        ...
        {
            action.filter = converter::convert_binary_to_filter(
                std::get<sql::BinaryExpr>(stmt.where->value),
                action.table
            );
        }
