...
namespace exe {
    ...
    ActionExecutor::ActionExecutor(engine::EngineConfig cfg)
        : cfg_(cfg), storage_() {
    }
    ...
    ActionExecutor::execute_action(const InsertAction& action) noexcept {
        try {
            ...
            return ActionExecutionResult(
                std::make_pair(
                    std::string("Insert execution error: ") + e.what(), 
    ...
    ActionExecutor::execute_action(const UpdateByFilterAction& action) noexcept {
        try {
            ...
            return ActionExecutionResult(
                std::make_pair(
                    std::string("Update execution error: ") + e.what(), 
                    ActionError::SYSTEM_ERROR
                )
            );
        }
    ...
    ActionExecutor::execute_action(const DeleteByFilterAction& action) noexcept {
        try {
            ...
            return ActionExecutionResult(
                std::make_pair(
                    std::string("Delete execution error: ") + e.what(), 
                    ActionError::SYSTEM_ERROR
                )
            );
        }
