...
namespace converter {
    DataRowUpdate create_row_update(const MetaTable &table, const sql::UpdateStatement &query) {
        ...
        for (size_t i = 0; i < assignments_count; i++) {
            ...
            const sql::SqlToken &left =
                std::get<sql::SqlToken>(assignment.left->value);
            const sql::SqlToken &right =
                std::get<sql::SqlToken>(assignment.right->value);
        ...
        DataRowUpdate update = {.count = assignments_count,
    ...
    DataRow convert_insert_to_data_row(const MetaTable& table,
        ...
        for (uint64_t i = 0; i < table.columns_count; ++i) {
            ...
            if (insert_idx == -1) {
                ...
                const auto value =
                    convert_str_to_literal(value_token.value, col->data_type);
                row.tokens[i] =
                    make_token(col->data_type, value.first, value.second);
            }
        }
    ...
    MetaTable convert_create_table_to_mt(const sql::CreateTableStatement& stmt) {
        return meta::init_meta_table(stmt.name.value, stmt.columns);
    }
}