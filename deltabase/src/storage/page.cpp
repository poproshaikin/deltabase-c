...
namespace storage {
    DataPage::DataPage(
        std::string id, std::string table_id, uint64_t size, RowId min_rid, RowId max_rid
    )
        : id_(id), table_id_(table_id), size_(size), min_rid_(min_rid), max_rid_(max_rid)
    {
    }
    ...
    DataPage::get_row(RowId rid)
    {
        ...
        throw std::runtime_error(
            std::format("Row with id {} was not found in DataPage {}", rid, id_)
        );
    }
